...
function setupCanvas(canvasElement, analyserNodeRef) {
    ...
    gradient = utils.getLinearGradient(ctx, 0, 0, 0, canvasHeight, [{ percent: 0, color: "red" }, { percent: .25, color: "purple" }, { percent: .5, color: "darkblue" }, { percent: .75, color: "black" }, { percent: 1, color: "black" }]);
...
function draw(params = {}, spriteParams1 , spriteParams2) {
    ...
    if (true == true) {
        ...
        for (let i = 0; i < audioData.length; i++) {
            ...
            ctx.beginPath();
            ctx.fillStyle = utils.makeColor(255, 111, 111, .34 - percent / 3.0);
            ctx.arc( spriteParams1.x , canvasHeight / 2, circleRadius, 0, 2 * Math.PI, false);
            ctx.fill();
            ctx.closePath();
